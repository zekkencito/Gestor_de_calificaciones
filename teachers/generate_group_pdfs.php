...
// Log para depuración
error_log("Iniciando generación de PDFs grupales para teachers");
...
$schoolYear = isset($_POST['schoolYear']) ? intval($_POST['schoolYear']) : 0;
...
if (!$stmtTeacher) {
    echo json_encode(['success' => false, 'message' => 'Error preparando consulta teacher: ' . $conexion->error]);
    exit;
}
...
if (!$rowTeacher) {
    ...
    die('No se pudo cargar la información del docente');
}
...
try {
    ...
    $stmtGroupInfo->bind_param("ii", $schoolYear, $grupo);
    ...
    $stmtQuarters->bind_param("i", $schoolYear);
    ...
    // Usar el primer trimestre disponible
    $quarterId = $quarters[0];
    
    // Obtener estudiantes del grupo asignados al teacher
    $sqlStudents = "SELECT DISTINCT s.idStudent, ui.names, ui.lastnamePa, ui.lastnameMa 
                    FROM students s 
                    JOIN usersInfo ui ON s.idUserInfo = ui.idUserInfo 
                    JOIN teacherGroupsSubjects tgs ON s.idGroup = tgs.idGroup
                    WHERE s.idGroup = ? AND s.idSchoolYear = ? AND tgs.idTeacher = ?
                    ORDER BY ui.lastnamePa, ui.lastnameMa, ui.names";
    
    $stmtStudents = $conexion->prepare($sqlStudents);
    $stmtStudents->bind_param("iii", $grupo, $schoolYear, $idTeacher);
    ...
    if (empty($students)) {
        error_log("No se encontraron estudiantes para grupo: $grupo, año: $schoolYear, teacher: $idTeacher");
        http_response_code(404);
        die('No se encontraron estudiantes en este grupo asignados a usted');
    }
