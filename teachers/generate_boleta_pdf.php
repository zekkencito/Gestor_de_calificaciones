...
function generateStudentPDF($idStudent, $idSchoolYear, $idSchoolQuarter, $conexion) {
    ...
    // Columna "Promedio Final" solo aparece cuando se muestran TODOS los bimestres
    ...
    $learningAreas = [];
    // Acumuladores: suma y conteo POR BIMESTRE (solo calificaciones reales, no nulas)
    ...
    while ($subject = $resultSubjects->fetch_assoc()) {
        // Calificación por bimestre — null si no hay calificación capturada
        $gradesByQ = [];
        foreach ($quartersToShow as $q) {
            ...
            $stmtAvg->close();
            // null = sin calificación (celda en blanco); 0 solo si la nota real es 0
        ...
        // Promedio final de la materia: solo sobre bimestres con calificación real
    ...
    // Promedios por área — null si ninguna materia tiene calificación en ese bimestre
    ...
    // Promedios generales por bimestre — null si no hay ninguna calificación en ese bimestre
    $generalAvgByQ  = [];
    $nonNullGenAvgs = [];
    foreach ($quartersToShow as $q) {
        $qid = (int)$q['idSchoolQuarter'];
    ...
    // Calcular anchos de columna dinámicamente según el número de bimestres a mostrar
    $totalW      = 176; // ancho útil (216mm - márgenes 20+20)
    $numGradeCols = count($quartersToShow) + ($showFinalAverage ? 1 : 0);
    // Columna de materia más ancha cuando hay menos bimestres
    ...
    // Filas por área y materia
    $pdf->SetTextColor(0, 0, 0);
    foreach ($learningAreas as $area) {
        // Fila de campo formativo (área)
